...
class TestRectifiedFlowCoefficients:
...
class TestRectifiedFlowTimeSampling:
...
class TestRectifiedFlowSpatialMethods:
...
class TestRectifiedFlowGetDenoiser:
    ...
    def test_validates_multiple_predictors(self, device):
        ...
        with pytest.raises(ValueError, match="Exactly one"):
            s.get_denoiser(flow_predictor=pred, x0_predictor=pred)
        with pytest.raises(ValueError, match="Exactly one"):
            s.get_denoiser()
    ...
    def test_sde_denoiser_shape(self, device):
        ...
        t = torch.full((BATCH,), 0.5, device=device)
        flow_pred = lambda x, t: -x  # noqa: E731
        denoiser = s.get_denoiser(flow_predictor=flow_pred, denoising_type="sde")
        out = denoiser(x, t)
...
class TestRectifiedFlowSampling:
    """End-to-end smoke test: flow-predictor -> denoiser -> sampler."""

    def test_sample_from_flow_predictor(self, device):
        s = RectifiedFlowNoiseScheduler()
        model = instantiate_model_deterministic(
            Conv2dX0Predictor, seed=0, channels=3
        ).to(device)
        denoiser = s.get_denoiser(flow_predictor=model)
        t_steps = s.timesteps(NUM_STEPS, device=device)
